...
```{r}
library(R6)
InputWarnings <- R6Class(
  "InputWarnings",
  public = list(
    warnings = list(),
    add = function(code, dutch, english, National) {
      msg <- if (National == "Nederlands") dutch else english
      self$warnings[[length(self$warnings) + 1]] <- list(code = code, message = msg)
    }
  )
)
inputwarnings <- InputWarnings$new()
...
```{r}
library(R6)
  InputWarnings <- R6Class( "InputWarnings",
    public = list(
    warnings = list(),
    add = function(code, dutch, english, National) {
    msg <- if (National == "Nederlands") dutch else english
...
# Example data
inputData <- data.frame(
  Parameter.code = c("SULFATE", "", "WATER", "50-00-0", "INVALIDCAS"),
  Parameter.CASnummer = c("7664-93-9", "7732-18-5", "7732-18-5", NA, "123-45-6"),
  stringsAsFactors = FALSE
)

SSDbron <- data.frame(
  CAS = c("7664-93-9", "7732-18-5", "50-00-0"),
  stringsAsFactors = FALSE
)
...
# Map and validate
result <- check_substance_codes(inputData, OtherChar, SSDbron, inputwarnings, National = "English")
...
# Example data
inputData <- data.frame(
  Parameter.code = c("CHEM1", "CHEM2", "CHEM3", "CHEM4"),
  Parameter.CASnummer = c("1111-11-1", NA, "3333-33-3", "4444-44-4"),
  Eenheid.code = c("mg/L", "ug/l", "mol/L", "mg/Lx"),
  Numeriekewaarde = c(1, 2, 0.001, 5),
  Hoedanigheid.code = c("W", "N", "Pnf", ""),
  stringsAsFactors = FALSE
)

SSDbron <- data.frame(
  CAS = c("1111-11-1", "2222-22-2", "3333-33-3"),
  MW.g.Mol = c(100, 50, 30),
  stringsAsFactors = FALSE
)
